...
def click_event(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        strang = str(x) + ", " + str(y)
        cv2.putText(img, strang, (x,y), font, 0.5, (255,255,0), 2)
    ...
    if event == cv2.EVENT_RBUTTONDOWN:
        ...
        cv2.putText(img, strang, (x,y), font, 0.5, (0,255,255), 2)
        cv2.imshow("image", img)
...
if mode == 'beep':#'optical':
    ...
    while(len(refPt) < 1): # could change it to be a number of points
        _,img = cap.read()
    ...
    p0 = np.array([[refPt[0]]], dtype=np.float32)
...
### THIS ALSO ISN'T WORKING AND IT'S COPY PASTE
if mode == 'optical':
    ...
    #pz = cv.goodFeaturesToTrack(old_gray, mask=None, **feature_params)

    ## Get singular mouseclick from camera
    #  - opening camera early to get target point to be tracked
    while(len(refPt) < 1):
        _,img = cap.read()
        cv2.namedWindow("image")
        cv2.setMouseCallback("image", click_event)
    ...
    #(heig, widt, _) = old_frame.shape
    #p0 = np.array([[[heig/2,widt/2]]], dtype=np.float32)
